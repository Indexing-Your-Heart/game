...
import Foundation
@testable import Paintbrush
import XCTest

class ProtractorTests: XCTestCase {
    func hundredPoints() -> [PaintbrushPoint] {
        var points = [PaintbrushPoint]()
        for iter in 1 ... 100 {
            points.append(PaintbrushPoint(x: Double(iter), y: Double(iter)))
        }
        return points
    }

    func rectPoints() -> [PaintbrushPoint] {
        var points = [PaintbrushPoint]()
        let width = 100
        let height = 75
    ...
    func testResamplingContainsCorrectNumberOfPoints() throws {
        ...
        let path = PaintbrushPointPath(points: points)
    ...
    func testRecognizerTemplateLoadsFromConfigurationFile() throws {
        let recogniser = PaintbrushRecognizer(from: .init(points: rectPoints()))
        recogniser.insertTemplates(reading: "templates", in: .module)
